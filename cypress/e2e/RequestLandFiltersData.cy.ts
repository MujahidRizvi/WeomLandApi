describe('Get User Info. api', function () {
  context('GET /Get User Info.', () => {
    ...
    it('should return 200 OK status when user inputs valid parameters', function () {
      cy.request({ method: this.testdata.get, url: this.testdata.searchFilterUrl, headers: { "Cookie": this.testdata.btoken }, failOnStatusCode: false }).then(
        (response) => {
    ...
    it('should have mentioned keys within data when user inputs valid parameters', function () {
      cy.request({ method: this.testdata.get, url: this.testdata.searchFilterUrl, headers: { "Cookie": this.testdata.btoken }, failOnStatusCode: false }).then(
        (response) => {
          expect(response.body.data).to.have.keys(this.testdata.searchFilterData)
        })
    })
    ...
    it('should have mentioned keys within data when user inputs valid parameters', function () {
      cy.request({ method: this.testdata.get, url: this.testdata.searchFilterUrl, headers: { "Cookie": this.testdata.btoken }, failOnStatusCode: false }).then(
        (response) => {
          expect(response.body.data.panel[0]).to.have.keys(this.testdata.panelData)
        })
    })
    ...
    /* it('should display success as TRUE when user inputs valid parameters', function () {
     ...
     it('user does not provide token, then 500 code should be returned in response', function () {
       cy.request({ method: this.testdata.get, url: this.testdata.searchFilterUrl, headers: {}, failOnStatusCode: false }).then(
         (response) => {
